...
[ff,muf]=DiscretePreisach(ncount,datainput,mu,n);
 ftable=[ftable ff];
   ...
   uNew=datainput(ncount);
    ...
    subplot(1,3,3);
     ...
     h = plot(NaN,NaN); 
      ...
      hLine = line('XData',datainput(1), 'YData',ftable(1), 'Color','r', ...
    'Marker','o', 'MarkerSize',6, 'LineWidth',2);
hTxt = text(datainput(1), ftable(1), sprintf('(%.3f,%.3f)',datainput(1),ftable(1)), ...
    ...
    'HorizontalAlignment','left', 'VerticalAlignment','top');
     ...
     axis([-a0 a0 -N N]); 
        ...
        pause(0.01)
        set(h, 'XData', datainput(2:ncount), 'YData', ftable(2:ncount));
         
        
     set(hLine, 'XData',datainput(ncount), 'YData',ftable(ncount))   
    set(hTxt, 'Position',[datainput(ncount) ftable(ncount)], ...
        'String',sprintf('(%.3f,%.3f)',[datainput(ncount) ftable(ncount)]))        
    drawnow 
